...
public:
	...
	/*解析客户端请求时，主状态机的状态
	CHECK_STATE_REQUESTLINE:当前正在分析的请求行
	CHECK_STATE_HEADER：当前正在分析头部字段
	CHECK_STATE_CONTENT :当前正在分析解析请求体
	*/
    enum CHECK_STATE
    {
...
private:
	...
	char m_read_buf[READ_BUFFER_SIZE]; //读缓冲里面数据
	int m_read_idx;						//标识读缓冲区中以及读入客户端数据的最后一个字节的下一个位置

	int m_checked_index; 	// 标识读缓冲区中以及读入的客户端数据的最后一个字节的下一个位置
	...
	int m_content_length; //请求行中消息体的长度

	char m_real_file[FILENAME_LEN]; //客户请求目标文件的完整路径，其内容等于doc_root+m_url,doc_root为网站根目录
