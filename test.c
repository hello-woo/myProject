...
在 C99 则加入了 "%zu"，但 VS2015 中才有，之前的 VC 版本使用非标准的 "%Iu" 
因此，代码使用条件编译去区分 VC 和其他编译器*/
...
/*
优化test_pares_expect_value 代码，增加宏 
 */
...
static void test_parse_object(){
    ...
    EXPECT_EQ_INT(LEPT_OBJECT,lept_get_type(lept_get_object_value(&v,6)));
    EXPECT_EQ_SIZE_T(3,lept_get_object_size(lept_get_object_value(&v,6)));
    o = lept_get_object_value(&v,6);
    for(i = 0 ;i < 3; i++){
...
int main() {
  ...
  /*   printf("this is the sizeof lept_value :%d %d %d\n ",(int)sizeof(lept_value),(int)sizeof(v.u),(int)sizeof(lept_type)); */
    return main_ret;
}